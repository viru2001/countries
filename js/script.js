...
request.addEventListener('load',  function() {
    ...
    for (i = 0; i < data.length; i++) {
        ...
        myData[data[i].name] = [data[i].flag, data[i].capital,data[i].languages[0].nativeName,data[i].population,
        data[i].demonym,data[i].currencies[0].name];
        countryIndex.push({value:data[i].name,data:i });
        htmlData += `
        <div class="box">
            <span></span>
    ...
    // console.log(myData);
    // console.log("ljidsjijdos",countryIndex);
});
...
// console.log(typeof(selectedCountryData),typeof(myData));
    // setup autocomplete function pulling from selectedCountryData[] array
    $('#autocomplete').autocomplete({
        lookup: countryIndex,
        onSelect: function (suggestion) {
            // console.log(suggestion.value,suggestion.data);
            // console.log(myData[suggestion.value]);
            let selectedCountryData = myData[suggestion.value];
            let htmlData1 = `
            <div class="box">
            <span></span>
    
            <div class="content">
                <img class="center" src="${selectedCountryData[0]}" />
                <h2 class="text-center">${suggestion.value}</h2>
                <p class="text-center">
                    Capital : ${selectedCountryData[1]}
                </p>
                <hr class="divider">
                ...
                <div class="row">
                    ...
                    <p class="col-md-6">
                        ${selectedCountryData[2]}<br>Language
                    </p>
                    <p class="col-md-6">
                        ${selectedCountryData[3]}<br>Population
                    </p>
                    <p class="col-md-6">
                        ${selectedCountryData[4]}<br>Demonym
                    </p>
                    <p class="col-md-6">
                    ${selectedCountryData[5]}<br>Currency
                    </p>
            ...
            `;
            container.innerHTML = "";
            container.insertAdjacentHTML('afterbegin', htmlData1);
        }
    });
...
let close = document.querySelector('.errspan');
let inputWrapper = document.querySelector('.input-wrapper') ;
let searchBox = document.querySelector('#autocomplete');
